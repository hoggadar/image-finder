        ...
        *,
            ...
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(
                        ...
                        extra={"url": url, "error": str(exc), "attempts": self.max_retries}
                    )
                    raise RuntimeError(f"Failed to reach service at {url} after {self.max_retries} attempts: {exc}") from exc
            except httpx.RequestError as exc:
                logger.exception("Failed to reach downstream service", extra={"url": url, "error": str(exc)})
        ...
        if response.status_code >= 400:
            ...
            logger.warning(
                ...
                extra={"status_code": response.status_code, "url": url, "payload": payload}
            )
            raise RuntimeError(f"Service returned error {response.status_code}: {payload}")
